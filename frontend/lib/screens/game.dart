...
class _GameScreenState extends State<GameScreen> {
  ...
  void _updateState(Map<String, dynamic> payload) {
    ...
    // FIX 2: Grouping the Center Pool
    if (center != null) {
      center!.sort((a, b) {
  ...
  Widget _buildTile(String colorName, {double size = 20, double opacity = 1.0, bool isGhost = false, bool empty = false, double scale = 1.0}) {
    ...
    // FIX 3: The Juice (Animated Switcher for pop-in effect on scoring)
                  ...
                  child: Row(
                    ...
                    children: [
                      Column(
                        ...
                        children: [
                          // FIX 5: Sub-header removed. Clean.
                              ...
                              children: List.generate(
                                ...
                                (r) => Row(
                                  ...
                                  children: List.generate(
                                    5,
                                    (c) {
                                      // FIX 6: Explicit Opponent Wall Mapping
                ...
                Expanded(flex: 5, child: Container(
                  ...
                  child: Column(
                    children: [
                                    ...
                                    return GestureDetector(
                                      ...
                                      child: Container(
                                        ...
                                        child: Row(
                                          mainAxisSize: MainAxisSize.min,
                                          children: [
                                            // FIX 1: Right-to-Left Packing Math
                                            Row(
                                              mainAxisSize: MainAxisSize.min,
                                              children: List.generate(5, (cIdx) {
                                                ...
                                                int capacity = rIdx + 1;
                                                int filledCount = (patternLines[rIdx] as List).where((s) => s != "").length;
                                                int emptyCount = capacity - filledCount;
                                                String rowColor = filledCount > 0 ? (patternLines[rIdx] as List).firstWhere((s) => s != "") : "";

                                                int ghostStartIdx = emptyCount;
                                                if (isHovered && isLegal && heldCount != null) {
                                                  ghostStartIdx = emptyCount - heldCount!;
                                                  if (ghostStartIdx < 0) ghostStartIdx = 0;
                                                }

                                                if (slotIdx < ghostStartIdx) return _buildTile("", size: 24, empty: true);
                                                else if (slotIdx >= ghostStartIdx && slotIdx < emptyCount) return _buildTile(heldColor!, size: 24, isGhost: true);
                      ...
                      const SizedBox(height: 12),
                      // FIX 4: BOTTOM BUTTON COMPLETELY REMOVED! (Space freed up)
                    ],
                  ),
                )),
              ],
