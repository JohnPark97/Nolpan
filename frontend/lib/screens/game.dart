...
class _GameScreenState extends State<GameScreen> {
  ...
  // --- UI COMPONENTS ---
  Widget _buildTile(String colorName, {double opacity = 1.0, bool isGhost = false, double scale = 1.0, bool empty = false}) {
    // THE FIX: Restored the empty tile rendering block
    if (empty) {
    ...
    switch (colorName) {
      ...
      case 'first_player': 
        return Container(width: 22, height: 22, margin: const EdgeInsets.all(1.5), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(4), border: Border.all(color: tGold, width: 2)), child: const Center(child: Text("1", style: TextStyle(color: tGold, fontWeight: FontWeight.bold, fontSize: 12))));
      ...
      child: Opacity(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: opacity == 1.0 && !isGhost ? [const BoxShadow(color: Colors.black12, offset: Offset(0, 2))] : [],
          ),
      ...
      body: SafeArea(
        child: Stack(
          children: [
                    ...
                    child: Column(
                      children: [
                              ...
                              children: List.generate(5, (rIdx) {
                                ...
                                return GestureDetector(
                                  onTap: isLegal ? () {
                                    ...
                                    if (heldColor != null && sourceKilnIdx != null) {
                                      // DIRECT PLACEMENT: Tap row to send to server
                                      socketService.send('PICK_TILES', {
                                        'code': socketService.currentRoomCode,
                                        'player': socketService.playerName,
                                        'kiln_idx': sourceKilnIdx,
                                        'color': heldColor,
                                        'target_row': rIdx
                                      });
                                      setState(() { heldColor = null; hoveredRow = null; sourceKilnIdx = null; });
                                    }
                                  } : () {
                                  ...
                                  child: Container(
                                    ...
                                    child: Row(
                                      children: List.generate(rIdx + 1, (cIdx) {
                                        ...
                                        // THE FIX: Strict typing for the Ghosting Math
                                        ...
                                        return _buildTile("", empty: true); // Uses the restored empty flag
                                      }),
                                    ),
                                  ),
                                );
                        ...
                        Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(7, (i) {
                          ...
                          return _buildTile(t, empty: t == "");
                        })),
                        const SizedBox(height: 12),
                        PhysicsButton(
                          text: heldColor != null ? "TAP ROW TO PLACE" : "SELECT TILES",
            ...
            // UNDO OVERLAY
            if (heldColor != null) Positioned.fill(child: GestureDetector(onTap: () => setState(() { heldColor = null; sourceKilnIdx = null; }), child: Container(color: Colors.transparent))),
          ],
        ),
      ),
    );
