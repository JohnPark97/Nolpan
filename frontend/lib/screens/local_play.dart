...
class _LocalPlayScreenState extends State<LocalPlayScreen> {
  ...
  // GAME ENGINE STATE (Local Memory)
  ...
  void _commitTurn(int targetRow) async {
    ...
    // Wait for flight to land before updating state arrays
    ...
    if (kIdx >= 0) {
      ...
      for (String t in center) {
        if (t == cColor) {
          ...
          if (b['floor_line'].length < 7) b['floor_line'].add("first_player");
          else _gameState['discard'].add("first_player");
        } else {
          newCenter.add(t);
        }
      }
      ...
      for (int i = 0; i < picked; i++) {
        if (emptySlots > 0) {
          for (int j = 0; j < rTiles.length; j++) {
            if (rTiles[j] == "") { rTiles[j] = cColor; emptySlots--; break; }
          }
        } else {
          if (b['floor_line'].length < 7) b['floor_line'].add(cColor);
          else _gameState['discard'].add(cColor);
        }
      }
    } else {
      for (int i = 0; i < picked; i++) {
  ...
  void _scoreRound() async {
    ...
    for (int r = 0; r < 5; r++) {
      if (b['pattern_lines'][r].where((s) => s == "").isEmpty && b['pattern_lines'][r].isNotEmpty) validScoringRows.add(r);
    }
    ...
    // --- STRICT MATH AUDIT: Positives -> Penalties -> Clamp ---
    for (String p in _localPlayers) {
      ...
      for (int i = 0; i < board['floor_line'].length; i++) {
        ...
        if (t == "first_player") _gameState['turn_player'] = p;
        else _gameState['discard'].add(t);
        if (i < pens.length) board['score'] += pens[i]; else board['score'] -= 3;
      }
                ...
                children: List.generate(4, (i) {
                  if (i < _localPlayers.length) {
                    return Padding(
                      ...
                      child: Column(
                        children: [
                          CircleAvatar(radius: 20, backgroundColor: _avatarColors[i % 4], child: Text(_localPlayers[i][0].toUpperCase(), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
                    ...
                    if (_isAddingPlayer) {
                        ...
                        child: TextField(
                          ...
                          onSubmitted: (val) {
                            if (val.trim().isNotEmpty) {
                              setState(() { _localPlayers.add(val.trim()); _isAddingPlayer = false; _nameCtrl.clear(); });
                            }
                          },
                        ),
                      );
                      ...
                      return GestureDetector(
                        ...
                        child: Container(margin: const EdgeInsets.symmetric(horizontal: 6), width: 40, height: 40, decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Colors.grey[300]!, width: 2)), child: const Icon(Icons.add, color: Colors.grey)),
                      );
                    }
                  } else {
                    return Container(margin: const EdgeInsets.symmetric(horizontal: 6), width: 40, height: 40, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.grey[100]));
                  }
                }),
              ),
            ),
  ...
  Widget build(BuildContext context) {
    if (_inLobby) return Scaffold(backgroundColor: tBg, body: SafeArea(child: _buildLobby()));
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      AnimatedSwitcher(
                        duration: const Duration(milliseconds: 300), switchInCurve: Curves.easeOutBack,
                        transitionBuilder: (child, animation) => ScaleTransition(scale: animation, child: child),
                        child: Container(
                          key: ValueKey(_turnPlayer),
                          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4),
                          decoration: BoxDecoration(color: _avatarColors[_localPlayers.indexOf(_turnPlayer) % 4], borderRadius: BorderRadius.circular(12), boxShadow: [BoxShadow(color: Colors.black26, offset: const Offset(0, 2), blurRadius: 4)]),
                          child: Text("TURN: " + _turnPlayer.toUpperCase(), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 10, letterSpacing: 1)),
                        )
                      ),
                ...
                Expanded(flex: 22, child: Container(
                  ...
                  child: Row(
                    ...
                    children: opponents.map((opp) {
                      ...
                      return Expanded(
                        child: Container(
                          margin: const EdgeInsets.symmetric(horizontal: 4), padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 8),
                          decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 10)]),
                          child: Column(
                            children: [
                              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Expanded(child: Row(children: [CircleAvatar(radius: 10, backgroundColor: _avatarColors[_localPlayers.indexOf(opp) % 4], child: Text(opp[0].toUpperCase(), style: const TextStyle(fontSize: 10, color: Colors.white, fontWeight: FontWeight.bold))), const SizedBox(width: 4), Expanded(child: Text(opp, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis))])), Text((board['score'] ?? 0).toString(), style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w900))]),
                              ...
                              Expanded(
                                child: FittedBox(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      Column(crossAxisAlignment: CrossAxisAlignment.end, children: List.generate(5, (r) => Row(mainAxisAlignment: MainAxisAlignment.end, children: List.generate(5, (cIdx) {
                ...
                Expanded(flex: 33, child: Column(
                  ...
                  children: [
                        ...
                        children: List.generate(_gameState['factories'].length, (kIdx) {
                          ...
                          return Opacity(
                            ...
                            child: Container(
                              ...
                              child: Center(child: Wrap(spacing: 2, runSpacing: 2, children: fTiles.map((c) {
                                bool isHeldLocally = heldColor == c && heldKilnIdx == kIdx;
                                bool dim = heldKilnIdx == kIdx && !isHeldLocally;
                                return GestureDetector(
                                  onTap: () { setState(() { heldColor = c; heldKilnIdx = kIdx; heldCount = fTiles.where((t) => t == c).length; }); },
                    ...
                    Container(
                      ...
                      child: Center(child: (_gameState['center'] as List).isEmpty ? const Text("CENTER POOL", style: TextStyle(fontSize: 10, color: Colors.grey)) : Padding(
                        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                        child: Wrap(spacing: 4, runSpacing: 4, children: (_gameState['center'] as List<String>).map((c) {
                          bool isHeldLocally = heldColor == c && heldKilnIdx == -1;
                          return GestureDetector(
                            onTap: c == "first_player" ? null : () { setState(() { heldColor = c; heldKilnIdx = -1; heldCount = (_gameState['center'] as List).where((t) => t == c).length; }); },
                            child: AnimatedContainer(
                              duration: const Duration(milliseconds: 150), curve: Curves.easeOutBack,
                              transform: isHeldLocally ? Matrix4.translationValues(0, -4.0, 0) : Matrix4.identity(),
                            ...
                            children: [
                              Column(
                                ...
                                children: List.generate(5, (rIdx) {
                                  bool isLegal = heldColor != null; // Simplified logic for brevity in pass-and-play port
                                  return GestureDetector(
                                    ...
                                    child: Row(
                                      children: List.generate(5, (cIdx) {
                                        ...
                                        return SizedBox(
                                          ...
                                          child: Stack(alignment: Alignment.center, children: [Positioned(child: t != "" ? _buildTile(t, size: 24) : _buildTile(wallPattern[rIdx][cIdx], size: 24, isGhost: true))])
                                        );
                                      }),
                                    ),
                                  );
